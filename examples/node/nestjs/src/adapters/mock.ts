import type { ChartIntent, ChartData, Dataset, Filter } from '../types';
import type { DataAdapter, DataRecord } from './base';
...
export class MockDataAdapter implements DataAdapter {
      ...
      return filters.every((filter) => {
        ...
        switch (filter.operator) {
          ...
          case 'lte':
            return typeof value === 'number' && typeof filterValue === 'number' && value <= filterValue;
          case 'in':
            return Array.isArray(filterValue) && (filterValue as (string | number)[]).includes(value as string | number);
          case 'between':
  ...
  private groupAndAggregate(data: DataRecord[], intent: ChartIntent): ChartData {
    ...
    const datasets = metrics.map((metric, idx) => ({
      ...
      backgroundColor:
        intent.chartType === 'pie' || intent.chartType === 'doughnut'
